...
const BuyerCategoryListing = () => {
    const route = useRoute();
    const dispatch = useDispatch();
    const navigation = useNavigation()
    const productsData = useSelector((state) => state?.buyer?.categoryProduct);
    const userData = useSelector((state) => state?.user?.user);
    const loader = useSelector((state) => state?.buyer?.loader);
    console.log(productsData, 'productsData')
    useEffect(() => {
        dispatch(BuyerCategoryProducts({ cat: route?.params?.category }))
    }, []);
    ...
    return loader ? <Loader /> : (
        <ScrollView contentContainerStyle={{ flex: 1, gap: 20 }}>
            <AppBar showBackArrow={true} showProfile={false} title={route?.params?.cat} navigation={navigation} />
            <FlatList
                data={productsData}
                scrollEnabled={false}
...
export default BuyerCategoryListing
const gapSize = 10
